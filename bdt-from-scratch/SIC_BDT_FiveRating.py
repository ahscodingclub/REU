...
def getTrees(head, t_features, np,nc,md,kround):
    param = [np,nc,md]
    with open("output/tree3.txt","r") as t:
        tree_file = t.read()
    trees = []
    clear = False
    if(len(tree_file)) != 0:
...
# MAIN SCRIPT
#####################################
f = open("output/BDT Output3.txt","w")
...
for trn_ind, tst_ind in kf:
    ...
    if choice_accy > k_accy:
        ...
        writeData("Testing", testhead, "output/TestOutput2.csv", "wb", actualTest, testLabel , confidence, credibility, len(trnLabels))
