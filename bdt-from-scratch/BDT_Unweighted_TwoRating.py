...
def getPigns(dataset):
    ...
    for case in dataset:
        ...
        pign[:] = [float(x) / (4.0 - zeroratings) for x in pign] 
...
def five_to_two(lst):
    theta = lst[2]/2
    pb = lst[0] + lst[1] + theta
    pm = lst[4] + lst[3] + theta
    return [pb,pm]
#####################################
# CREATING THE BDT
#####################################
...
def getAccuracy(class_matrix): # returns accuracy of misclassification matrix 
    accy = 0.0
    for j in range(0,len(class_matrix)):
    ...
    with open(filename, params) as csvfile:
        ...
        if trainOrTest == "Training":
            ...
            writer.writerow([
                             'Actual Benign',      'Actual Malignant', \
                             'Predicted Benign' ,  'Predicted Malignant',\
                             'RMSE', 'Pearson r', 'ROC AUC', 'Confidence', 'Credibility', 'Class', header])
        ...
        for i in range(0, len(predicted)):
            ...
            for j in range(0,2):
                ...
                xySum += (actual[i][j] - float(sum(actual[i]))/2.0) * (predicted[i][j] - float(sum(predicted[i]))/2.0)
                xSum += pow((actual[i][j] - float(sum(actual[i]))/2.0),2)
                ySum += pow((predicted[i][j] - float(sum(predicted[i]))/2.0), 2)
            rmse /= 2.0
            rmse = math.sqrt(rmse)
            if xSum == 0:
                xSum = 1
...
#            roc = 0.0
            ...
            if trainOrTest == "Training":
                ...
                writer.writerow([
                                 actual[i][0], actual[i][1],\
                                 predicted[i][0], predicted[i][1], \
                                 rmse, r, roc, conf[i], cred[i], classes[i]])
...
def getTrees(tf,head,np,nc,mind,md,switch):
    ...
    if switch:
        with open("output/tree.txt","wb") as t:
            tree = createTree(train_features, header, nparent, nchild, mind, maxdepth)
            tree = tree[0]
            trees = [param,tree]
...
#        print(i, "\tP-values: [", '{d[0]:.5} {d[1]:.5} {d[2]:.5} {d[3]:.5} {d[4]:.5}'.format(d=val), "]\tClass Label: ", getMax(testLabels[i]), "  Confidence: ", 1-sec, "\tCredibility: ", max(val), file=f)
        ...
        classes.append(val.index(max(val))+1)        
...
#        testROC.append(getROC(testLabels[i], actualTest[i]))
    ...
    testROC = writeData("Testing", testhead, "output/TestOutput1.csv", "wb", actualTest, testLabels, confidence, credibility, classes, accuracy, len(trainLabels))
