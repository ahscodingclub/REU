...
def five_to_two(lst):
    ...
    return [pb,pm]
#    print (lst, [(lst[0]+lst[1]+theta),(lst[3] + lst[4] + theta)])
#    return [(lst[0]+lst[1]+theta),(lst[3] + lst[4] + theta)]
#####################################
# CREATING THE BDT
#####################################
...
def PValue(test, calib):
    ...
    for i in range(len(test)): # for each case in testing data
        counts = [0,0,0] # stores count of test conformity greater than equal to calib 
    ...
    with open(filename, params) as csvfile:
        ...
        for i in range(0, len(predicted)):
            ...
            for j in range(2):
                ...
                xySum += (actual[i][j] - sum(actual[i])/ratings) * (predicted[i][j] - sum(predicted[i])/2)
